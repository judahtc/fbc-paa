...
@Component({
  selector: 'app-filenames-arc',
  templateUrl: './filenames-arc.component.html',
  styleUrls: ['./filenames-arc.component.scss']
})
export class FilenamesArcComponent {
  error: any=null;
...
arc_all_results_download(){
  this.showSpinner4()
  let id=localStorage.getItem('id')

  this.createProjectService.arc_all_results_download(localStorage.getItem('tn'),id).subscribe({next:response=>{
    let filename=response.headers.get('content-disposition')?.split(';')[1].split('=')[1];
    let blob:Blob = response.body as Blob;
    let a = document.createElement('a');
    a.download='project_'+localStorage.getItem('id')+'_arc';
